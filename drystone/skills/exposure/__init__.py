...
class ExposureSkill(BaseSkill):
    ...
    def _apply_exposure_post_processing(self, session: AuditSession, findings_path: Path) -> None:
        ...
        # Deterministic exposure findings are generated by BaseSkill pre-check
        # reconciliation before the JSON is saved. Do not re-run the legacy
        # S3-only generator here: it has less traceability and can overwrite
        # enriched EXP-013/EXP-014 findings with null impact/minimal refs.
